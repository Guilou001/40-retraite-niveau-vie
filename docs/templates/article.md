...
Sur {{paths}} départs américains complets de 40 ans, le retrait proportionnel n'épuise aucun portefeuille. Les dépenses passent pourtant sous 75 % du budget initial dans {{percentage_shortfall}} % des cas. Le retrait constant épuise le capital dans {{fixed_ruin}} % des cas. L'ajustement limité réduit ces difficultés, sans les supprimer. Les résultats internationaux et simulés empêchent de transformer une réussite historique américaine en garantie générale.
...
Notre question est plus précise. Une règle qui améliore la survie du portefeuille protège-t-elle aussi un budget minimal, et pendant combien d'années ce budget peut-il manquer ? Le projet complète le [projet 12 sur l'épargne](https://github.com/Guilou001/12-plan-epargne) en passant de l'accumulation aux retraits.

## 2. Ce qui vient du papier et ce qui change

| Élément | Papier de Vanguard | Cette étude |
| --- | --- | --- |
| Limites de l'ajustement annuel | Plancher de −1,5 % et plafond de +5 % dans l'exemple | Même mécanisme, testé sur les deux cas numériques de la figure 6 |
| Scénarios de rendement | Prévisions du modèle privé VCMM | Rendements historiques et rééchantillonnage de blocs d'années |
| Risque observé | Pérennité et flexibilité des dépenses | Épuisement, fréquence des réductions, durée et montant du manque |
| Budget minimal absolu | Ne constitue pas notre spécification principale reprise du papier | Ajout d'un seuil à 75 % des dépenses initiales |
| Diversification | Hypothèses du papier | Portefeuille domestique 50 % actions et 50 % obligations |
...
```text
Capital réel disponible = capital réel précédent × (1 + rendement nominal)
                         × (1 − frais annuels) / (1 + inflation)
Capital réel final = capital réel disponible − dépense réelle payée
```
...
![Deux mesures de difficulté](results/figures/budget_et_capital.png)
...
Le budget minimal réussit sur les trajectoires américaines de 40 ans retenues. Cette observation ne signifie pas que le seuil est garanti. Dans les scénarios de 40 ans reconstruits par blocs de cinq années, son capital s'épuise dans {{bootstrap_floor_ruin}} % des cas.
...
![Départ en 1966](results/figures/depart_1966.png)
...
![Comparaison internationale](results/figures/comparaison_pays.png)
...
![Sensibilité au rééchantillonnage](results/figures/sensibilite_blocs.png)
...
Le [protocole](docs/PROTOCOLE.md) précède le premier calcul complet mais n'est pas un préenregistrement externe. Les [sources](docs/DONNEES.md), leurs empreintes et les paramètres sont séparés du code. Les tests vérifient les exemples de Vanguard, l'inflation, les frais, l'ordre des rendements, les années manquantes et le plafonnement des retraits.

Les tables CSV alimentent les figures, le classeur et le texte. Le PDF est compilé depuis cet article. Le [journal de vérification](docs/VERIFICATION.md) détaille les contrôles effectivement réalisés.
...
The proportional rule avoids depletion in the observed US forty-year cohorts, but spending falls below the minimum in {{percentage_shortfall}}% of them. Constant real spending depletes the portfolio in {{fixed_ruin}}% of cohorts. These are overlapping historical observations, not independent trials or forecasts. Dynamic spending limits improve some outcomes while creating a different trade-off between flexibility and depletion risk.
...
Costa, P., Pakula, D. et Clarke, A. (2021). [Fuel for the F.I.R.E.](https://www.vanguardmexico.com/content/dam/intl/americas/documents/mexico/en/fuel-for-the-fire.pdf). Vanguard Research.

Jordà, Ò., Knoll, K., Kuvshinov, D., Schularick, M. et Taylor, A. M. (2019). [The Rate of Return on Everything, 1870–2015](https://doi.org/10.1093/qje/qjz012). Quarterly Journal of Economics, 134(3), 1225–1298. Données actualisées dans [JST R6](https://www.macrohistory.net/database/).
